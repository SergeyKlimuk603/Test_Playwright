...
let testDataJson = 'tests/Draft/register-business.json';
let OrgName  = 'DevHub_Dev';
let clearTestDataBefore = false;
...
test('create-new-account', async ({ page }) => 
{
	...
	console.log('testDataJObj: ', testDataJObj);
	...
	// let sfOutput = await exec('sf org:open -o '+OrgName+' --path /lightning/app/standard__LightningSales --url-only --json', execEnv);
	let sfOutput = await exec('sf org:open -o '+OrgName+' --path /lightning/app/standard__LightningSales -r --json', execEnv);
	var jsonObj = JSON.parse(sfOutput.stdout.trim());
